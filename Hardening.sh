#!/bin/bash
#Description: This script contain the CIS configuration use at Data center.
# Authro : Levi
...
cramps >> /etc/modprobe.d/CIS.conf
...
echo "net.ipv6.conf.default.accept_ra  "			
echo "net.ipv6.conf.all.accept_redirect = 0 "
echo "net.ipv6.conf.default.accept_redirect = 0 "
echo "net.ipv6.conf.all.disable_ipv6 = 0 "
}

limit.conf >> /etc/sysctl.conf
...
-a always,exit -F arch=b32 -S setxattr -S lsetxattr -S fsetxattr -S removexattr -S lremovexattr -S fremovexattr -F auid>=1000 -F auid!=4294967295 -k perm_mod-a always,exit -F arch=b64 -S creat -S open -S openat -S truncate -S ftruncate -F exit=-EACCES -F auid>=1000 -F auid!=4294967295 -k access
...
sed -i "s/\#LoginGraceTime 2m/LoginGraceTime 60/" ${sshd_config}	
